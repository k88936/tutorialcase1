...
这个产品工具免费，额度收费，但支持BYOK（bring your own key）我们不必注册，使用我们自己的`api key`.
...
* 学校的算力券  
    ...
    3. `model name` 是红色圈里的字符串 
        ![](docs/shotmd-20260420-012832.jpeg)
    4. `base url` 经过验证, 使用`https://llmapi.paratera.com`
    5. `api key` [申请](https://ai.paratera.com/#/lms/api)

* 阿里云
    > 提示 阿里云需要实名认证，嫌麻烦的同学使用学校的算力券就好.  
    > 阿里云对于大部分模型提供了90天一定量的免费额度，足够我们课程使用. ![](docs/shotmd-20260420-012329.jpeg)

    1. [模型选择页](https://bailian.console.aliyun.com/cn-beijing?tab=model#/model-market/all) 
    2. 详情页面
    3. `model name` 可以直接复制这里
        ![](docs/shotmd-20260420-012524.jpeg)
...
> 可以对比下两者的文档，良好的文档是很重要的

## config continue

1. 在vscode的插件管理中搜索并安装continue
2. 把`.continue/agents/config-example.yaml`复制一份到`.continue/agents/config.yaml`. 
3. 然后修改: 替换 `MODEL_NAME` `BASE_URL` `API_KEY` (yaml中的简单的字符串不必加引号)

> 我们做一次复制的目的是: 已经把`.continue/agents/config.yaml`加入gitignore中，避免同学不小心提交了配置文件导致 `api key`泄露。  
> 更安全的方法是使用环境变量，这里我们从简。

4. 然后我们在continue的配置选择里选择我们的config.yaml 
    ![](docs/shotmd-20260420-011414.jpeg)

## hello software engineering
![](docs/shotmd-20260420-010730.jpeg)